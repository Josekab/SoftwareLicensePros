import{Base,Polymer,dom,flush,html$1 as html,afterNextRender,IronResizableBehavior,useShadow,add,addListener,PolymerElement,html as html$1,mixinBehaviors,Debouncer,timeOut,microTask,IronSelectableBehavior}from"./shop-app.js";Polymer({_template:html`
    <style>
      :host {
        position: fixed;
        top: -120px;
...
if(event.defaultPrevented){return}}}},_triggerKeyHandler:function(keyCombo,handlerName,keyboardEvent){var detail=Object.create(keyCombo);detail.keyboardEvent=keyboardEvent;var event=new CustomEvent(keyCombo.event,{detail:detail,cancelable:!0});this[handlerName].call(this,event);if(event.defaultPrevented){keyboardEvent.preventDefault()}}};var ironA11yKeysBehavior={IronA11yKeysBehavior:IronA11yKeysBehavior};Polymer({is:"iron-request",hostAttributes:{hidden:!0},properties:{/**
...
}}},IronButtonState=[IronA11yKeysBehavior,IronButtonStateImpl];/** @polymerBehavior */var ironButtonState={IronButtonStateImpl:IronButtonStateImpl,IronButtonState:IronButtonState};const IronFitBehavior={properties:{/**
...
// align setting is ok.
if(0>diff||0===diff&&(vAlignOk||hAlignOk)){position=candidate}}return position}};var ironFitBehavior={IronFitBehavior:IronFitBehavior};Polymer({_template:html`
    <style>
      :host {
        display: block;
      }
...
`;template$3.setAttribute("strip-whitespace","");/**
                                                 ...
                                                 @hero hero.svg
                                                 @demo demo/index.html
                                                 */Polymer({_template:template$3,is:"paper-spinner-lite",behaviors:[PaperSpinnerBehavior]});const $_documentContainer$1=document.createElement("template");$_documentContainer$1.innerHTML=`<iron-iconset-svg style="display:none" name="icons" size="24">
<svg><defs>
...
export{appStorageBehavior as $appStorageBehavior,ironA11yKeysBehavior as $ironA11yKeysBehavior,ironButtonState as $ironButtonState,ironControlState as $ironControlState,ironFitBehavior as $ironFitBehavior,ironFocusablesHelper as $ironFocusablesHelper,ironMeta as $ironMeta,ironOverlayBehavior as $ironOverlayBehavior,ironOverlayManager as $ironOverlayManager,ironScrollManager as $ironScrollManager,paperInkyFocusBehavior as $paperInkyFocusBehavior,paperRippleBehavior as $paperRippleBehavior,paperSpinnerBehavior as $paperSpinnerBehavior,AppStorageBehavior,IronA11yKeysBehavior,IronButtonState,IronButtonStateImpl,IronControlState,IronFitBehavior,IronFocusablesHelper,IronMeta,IronOverlayBehavior,IronOverlayBehaviorImpl,IronOverlayManager,IronOverlayManagerClass,PaperInkyFocusBehavior,PaperInkyFocusBehaviorImpl,PaperRippleBehavior,PaperSpinnerBehavior,_boundScrollHandler,_composedTreeContains,_getScrollInfo,_getScrollableNodes,_getScrollingNode,_hasCachedLockedElement,_hasCachedUnlockedElement,_lockScrollInteractions,_lockedElementCache,_lockingElements,_scrollInteractionHandler,_shouldPreventScrolling,_unlockScrollInteractions,_unlockedElementCache,currentLockingElement,elementIsScrollLocked,pushScrollLock,removeScrollLock};