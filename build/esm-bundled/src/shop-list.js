...
return item.name?["/detail",this.category.name,item.name].join("/"):null}_getPluralizedQuantity(quantity){if(!quantity){return""}let pluralizedQ=1===quantity?"item":"items";return"("+quantity+" "+pluralizedQ+")"}_categoryChanged(category,visible){if(!visible){return}this._changeSectionDebouncer=Debouncer.debounce(this._changeSectionDebouncer,microTask,()=>{if(category){// Notify the category and the page's title
