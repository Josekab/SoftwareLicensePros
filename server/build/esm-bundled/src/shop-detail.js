import{PolymerElement,html,Debouncer,microTask}from"./shop-app.js";class ShopDetail extends PolymerElement{static get template(){return html`
    ...
    `}static get is(){return"shop-detail"}static get properties(){return{item:Object,route:Object,routeData:Object,visible:{type:Boolean,value:!1},offline:{type:Boolean,observer:"_offlineChanged"},failure:Boolean}}static get observers(){return["_itemChanged(item, visible)"]}_itemChanged(item,visible){if(visible){this._itemChangeDebouncer=Debouncer.debounce(this._itemChangeDebouncer,microTask,()=>{// The item description contains escaped HTML (e.g. "&lt;br&gt;"), so we need to
